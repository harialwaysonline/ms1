...
public class BookResource {
    ...
    @GET
    @Produces(MediaType.APPLICATION_JSON)
    @Path("/authors/{isbn: \\d{9}[\\d|X]$}")
    public Response getAuthors(final @PathParam("isbn") String isbn){


        Optional<Book> books=  bookRepository.getByISBN(isbn);
        if(!books.isPresent()){
            return Response.status(Response.Status.NOT_FOUND).build(); }
        List<Author> authors = books.get().getAuthors();
        if(authors.isEmpty()){
            return Response.status(Response.Status.NOT_FOUND).build();
        }
        GenericEntity ent = new GenericEntity<List<Author>>(authors){};
